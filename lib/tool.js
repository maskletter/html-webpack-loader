	...
	_remove_script(content){
		...
		for(let result of script){
			for(let data in result){
				let str = method.script_method.ts_transform(result[data].content);
				str = method.script_method.js_uglify(str).replace(/\'/g,single);
...
const method = {
	...
	script_method: {
		...
		ts_transform: function(str){
			...
			if(_ts != false){
				return tss(str, _ts ? Object.concat(tscofnig, _ts) : tscofnig);
			}
			return str;
		},
		//max--->min压缩
